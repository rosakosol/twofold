...
# TwofoldTokens (twofold-design-tokens@1.0.0)

This design system is the published twofold-design-tokens React library, bundled as a single
browser global. All 0 components are the real upstream code.

## Where things are

- `_ds_bundle.js` — the whole-DS bundle at the project root; loads every component to `window.TwofoldTokens`. First line is a `/* @ds-bundle: … */` metadata header.
...
Components are then available at `window.TwofoldTokens.*`. Mount into a dedicated child node (e.g. `<div id="ds-root">`), not the host page's own React root, so the two trees don't collide:

```jsx
const { Component } = window.TwofoldTokens;
ReactDOM.createRoot(document.getElementById('ds-root')).render(<Component />);
```

## Tokens

27 CSS custom properties from twofold-design-tokens. Names are
preserved verbatim from upstream. They are declared inside `_ds_bundle.css` (this DS ships one compiled stylesheet rather than separate token files).

- **color** (5): `--bg-top`, `--bg-bottom`, `--card-bg-alt`, …
- **typography** (2): `--font-display`, `--font-body`
- **radius** (4): `--radius-sm`, `--radius-card`, `--radius-lg`, …
- **shadow** (3): `--shadow-sm`, `--shadow-md`, `--shadow-lg`
- **other** (13): `--sky-blue`, `--sky-blue-light`, `--sky-blue-deep`, …
